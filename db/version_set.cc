...
void Version::AddLiveFiles(std::vector<uint64_t>* live_files) const {
  assert(live_files);
  ...
  for (int level = 0; level < storage_info_.num_levels(); ++level) {
    ...
    for (const auto& meta : level_files) {
      ...
      live_files->emplace_back(meta->fd.GetNumber());
    }
  }
}
...
void VersionSet::AddLiveFiles(std::vector<uint64_t>* live_files) const {
  assert(live_files);
  ...
  live_files->reserve(live_files->size() + total_files);
  ...
  for (auto cfd : *column_family_set_) {
    ...
    if (!found_current && current != nullptr) {
      ...
      assert(false);
      current->AddLiveFiles(live_files);
    }
  }
}
